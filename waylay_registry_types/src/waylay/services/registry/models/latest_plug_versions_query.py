...
class LatestPlugVersionsQuery(BaseModel):
    ...
    latest: Optional[StrictBool] = Field(default=None, description="When `true`, only the latest version per function name is returned. If set to `false`, multiple versions per named function can be returned. Defaults to `true`, except when specific versions are selected with the `nameVersion` filter.")
    __properties: ClassVar[List[str]] = ["tags", "type", "limit", "page", "includeDraft", "includeDeprecated", "deprecated", "draft", "nameVersion", "version", "status", "runtimeVersion", "createdBy", "updatedBy", "createdBefore", "createdAfter", "updatedBefore", "updatedAfter", "name", "archiveFormat", "runtime", "latest"]
