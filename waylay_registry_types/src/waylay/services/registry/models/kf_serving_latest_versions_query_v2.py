...
KFSERVINGLATESTVERSIONSQUERYV2_ANY_OF_SCHEMAS = [
...
class KFServingLatestVersionsQueryV2(BaseModel):
    ...
    def actual_instance_must_validate_anyof(cls, v):
        ...
        instance = KFServingLatestVersionsQueryV2.model_construct()  # noqa: F841
        error_messages = []
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when setting the actual_instance in KFServingLatestVersionsQueryV2 with anyOf schemas: LatestFunctionVersionsQuery, LatestFunctionsQuery. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if error_messages:
            # no match
            raise ValueError(
                "No match found when deserializing the JSON string into KFServingLatestVersionsQueryV2 with anyOf schemas: LatestFunctionVersionsQuery, LatestFunctionsQuery. Details: "
                + ", ".join(error_messages)
            )
        else:
            return instance
