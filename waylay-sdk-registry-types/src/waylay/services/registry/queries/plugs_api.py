...
def _get_asset_by_role_query_alias_for(field_name: str) -> str:
    if field_name == "ls":
        return "ls"
    return field_name


class GetAssetByRoleQuery(WaylayBaseModel):
    """Model for `get_asset_by_role` query parameters."""
    ...
    model_config = ConfigDict(
        protected_namespaces=(),
        extra="allow",
        alias_generator=_get_asset_by_role_query_alias_for,
        populate_by_name=True,
    )
...
def _get_latest_query_alias_for(field_name: str) -> str:
    ...
    if field_name == "include_undeployed":
...
class JobsQuery(WaylayBaseModel):
    ...
    limit: Annotated[
        Annotated[float, Field(strict=True, ge=0)] | Annotated[int, Field(strict=True, ge=0)] | None,
        Field(
...
def _patch_manifest_query_alias_for(field_name: str) -> str:
    if field_name == "show_tags":
        return "showTags"
    if field_name == "scale_to_zero":
        return "scaleToZero"
...
class PatchManifestQuery(WaylayBaseModel):
    """Model for `patch_manifest` query parameters."""
    ...
    model_config = ConfigDict(
        protected_namespaces=(),
        extra="allow",
        alias_generator=_patch_manifest_query_alias_for,
        populate_by_name=True,
    )
...
def _update_asset_by_role_query_alias_for(field_name: str) -> str:
    if field_name == "show_tags":
        return "showTags"
    if field_name == "scale_to_zero":
        return "scaleToZero"
...
class UpdateAssetByRoleQuery(WaylayBaseModel):
    """Model for `update_asset_by_role` query parameters."""
    ...
    model_config = ConfigDict(
        protected_namespaces=(),
        extra="allow",
        alias_generator=_update_asset_by_role_query_alias_for,
        populate_by_name=True,
    )
