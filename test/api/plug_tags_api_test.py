...
from waylay.services.registry.api import PlugTagsApi
...
if MODELS_AVAILABLE:
    ...
    from waylay.services.registry.queries.plug_tags_api import (
        ClearAllQuery,
        FindAllQuery,
        ListAllQuery,
        ReplaceAllQuery,
...
@pytest.fixture
def plug_tags_api(waylay_api_client: ApiClient) -> PlugTagsApi:
    return PlugTagsApi(waylay_api_client)


def test_registered(waylay_client: WaylayClient):
    """Test that PlugTagsApi api is registered in the sdk client."""
    assert isinstance(waylay_client.registry.plug_tags, PlugTagsApi)
...
def _add_all_set_mock_response(httpx_mock: HTTPXMock, gateway_url: str, name: str):
    ...
    httpx_mock_kwargs = {
        "method": "PATCH",
        "url": re.compile(f"^{gateway_url}/registry/v2/plugs/{name}/tags(\\?.*)?"),
...
def _add_set_mock_response(
    ...
    httpx_mock_kwargs = {
        "method": "PATCH",
        "url": re.compile(
            f"^{gateway_url}/registry/v2/plugs/{name}/versions/{version}/tags(\\?.*)?"
        ),
        "content": json.dumps(mock_response, default=str),
        "status_code": 200,
    }
...
def _find_all_set_mock_response(
    ...
    httpx_mock_kwargs = {
        "method": "GET",
        "url": re.compile(
            f"^{gateway_url}/registry/v2/plugs/{name}/tags/{tagName}(\\?.*)?"
        ),
        "content": json.dumps(mock_response, default=str),
        "status_code": 200,
    }
...
def _find_set_mock_response(
    ...
    httpx_mock_kwargs = {
        "method": "GET",
        "url": re.compile(
            f"^{gateway_url}/registry/v2/plugs/{name}/versions/{version}/tags/{tagName}(\\?.*)?"
        ),
        "content": json.dumps(mock_response, default=str),
        "status_code": 200,
    }
