"""Waylay Function Registry model tests.

This code was generated from the OpenAPI documentation of 'Waylay Function Registry'

Do not edit the class manually.
"""

import json

from jsf import JSF
from pydantic import TypeAdapter

from ..openapi import MODEL_DEFINITIONS, with_example_provider

try:
    from waylay.services.registry.models.rebuild_policy import RebuildPolicy

    RebuildPolicyAdapter = TypeAdapter(RebuildPolicy)
    MODELS_AVAILABLE = True
except ImportError:
    MODELS_AVAILABLE = False

rebuild_policy_model_schema = json.loads(
    r"""{
  "type" : "string",
  "description" : "The policy to select a new <em>runtime</em> version when a rebuild is issued.",
  "enum" : [ "patch", "minor", "major", "same" ]
}
""",
    object_hook=with_example_provider,
)
rebuild_policy_model_schema.update({"definitions": MODEL_DEFINITIONS})

rebuild_policy_faker = JSF(rebuild_policy_model_schema, allow_none_optionals=1)


class RebuildPolicyStub:
    """RebuildPolicy unit test stubs."""

    @classmethod
    def create_json(cls):
        """Create a dict stub instance."""
        return rebuild_policy_faker.generate(use_defaults=True, use_examples=True)

    @classmethod
    def create_instance(cls) -> "RebuildPolicy":
        """Create RebuildPolicy stub instance."""
        if not MODELS_AVAILABLE:
            raise ImportError("Models must be installed to create class stubs")
        json = cls.create_json()
        if json is None:
            # use backup example based on the pydantic model schema
            backup_faker = JSF(
                RebuildPolicyAdapter.json_schema(), allow_none_optionals=1
            )
            json = backup_faker.generate(use_defaults=True, use_examples=True)
        return RebuildPolicyAdapter.validate_python(
            json, context={"skip_validation": True}
        )
